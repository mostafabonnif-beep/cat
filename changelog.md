...
# إصدار OUSSAMA Cutter 7.41.0-pro — تحصين إلزامي لجودة الاختيار: تحقق موحّد نهائي، رفض النوافذ المعكوسة، تقييم غير موثّق، وحدود رقمية صريحة

## المشكلة (نتائج التدقيق الإلزامي على 7.40)

- **العنوان المُوصى به لم يكن يُتحقق منه دائماً**: العنوان الذي يرسله الـ LLM في `recommended_title` كان يُحفظ كما هو، والعنوان البديل (`alt_titles`) لم يكن يُختبر واقعياً إطلاقاً، فيصل عنوان بأرقام/أسماء/ادعاءات مخترعة أو عن الفيديو كامل إلى النشر.
- **الطوابع الرقمية الصريحة كانت موثوقة كما هي**: نافذة `start_time/end_time` الصريحة كانت تتجاوز تثبيت الحدود، فقد يبدأ/ينتهي القص داخل كلمة، ولا يوجد تحقق من مدة الوسائط الفعلية.
- **النافذة المعكوسة كانت تُعكس بصمت**: `end < start` كان يُبدَّل تلقائياً بلا دليل، ما يغيّر النافذة الدلالية المقصودة.
- **درجات تحريرية منسوخة**: `hook_strength/narrative_completeness/clarity_score` كانت تُنسخ من درجة الانتشار عند غيابها، ولا يوجد `quality_status` يميّز الحقيقي من غير الموثّق.
- **التمديد الأعمى للمقاطع القصيرة**: كان يُمدّ المقطع لأي كلام قريب للوصول للحد الأدنى، بلا فحص اتصال سياقي.
- **لا يوجد مُدقّق نهائي واحد** يُستدعى بعد المحاذاة وبعد scene-snap وقبل الحفظ/القص/النشر.

## الحل

- **`scripts/segment_validator.py` (جديد)** — مُدقّق نهائي واحد قابل لإعادة الاستخدام: أخطاء مُهيكلة (لا منطقي فقط) للطوابع، `end>start`، المدة الموجبة، الحد الأدنى/الأقصى، حدود الوسائط الفعلية، نص تفريغ غير فارغ، عدم قطع كلمة/جملة، صمت الأطراف، حالة العنوان، حالة الأمان، والتكرار الزمني/الدلالي. يُستدعى في `create_viral_segments` (بعد المحاذاة)، و`save_json` (قبل حفظ viral_segments.txt)، و`cut_segments` (بعد scene-snap وقبل FFmpeg)، و`upload_gate` (قبل النشر). أخطاء fatal تُمنع، وما دونها يُعلَّم للمراجعة اليدوية.
- **تحصين العناوين في `scripts/title_factual.py`**: تحقق موحّد لكل عنوان (`recommended_title` و`title` و`alt_titles` وtitles من `seo_titles` والواجهة والنشر) مقابل نص نافذة القص بالضبط، بمخطط `title_validation` (matches_clip_transcript، factual_consistency، contains_unsupported_entity/number/claim، language_matches_clip، clickbait_penalty، status: verified/review/rejected، reasons)، مع فحص الأرقام/الأسماء/الادعاءات/الإطار «الفيديو كامل»، وفحص استلزام اختياري LLM مع سقوط حتمي، وعنوان احتياطي محافظ من نص المقطع، وتعليم `title_review_required` بدل النشر الصامت. البدائل المهلوّسة تُستبعد من `alternative_titles`.
- **`seo_titles.generate_titles`** يدعم وضعاً مُقيّداً (`transcript_text`) يستبعد أي عنوان بأرقام/ادعاءات غير موجودة في المقطع.
- **الحدود الصريحة**: كل نافذة — بما فيها الطوابع الرقمية الصريحة — تمر بتثبيت حدود الكلمات/الجمل، شبكة أمان لحدود الكلمات حتى عند تعطيل التثبيت، تسجيل تعارض الحدود مع المدة كـ quality flag، وتحقق من `media_duration` الفعلي عبر ffprobe.
- **النوافذ المعكوسة**: تُستعاد من `start_text/end_text` عبر محاذاة ضبابية؛ وإن تعذّرت تُرفض بسبب `reversed_window` — لا عكس صامت.
- **`quality_status` + عدم نسخ درجة الانتشار**: المكوّنات التحريرية الناقصة تُخزَّن `null` وتُعلَّم `unverified`، والمرشّح غير الموثّق الذي يفشل فحص الاكتمال/الحدود يُعلَّم `requires_review` ولا يُنشر تلقائياً (بوابة `upload_gate`).
- **التمديد الذكي للمقاطع القصيرة**: بحث عند حدود الجمل مع فحص اتصال سياقي (كلمة مشتركة أو كلام متصل)؛ وإن تعذّر، يبقى المقطع قصيراً ويُعلَّم `transcript_limited`.
- **الأوزان الـ12 للمواصفة**: `narrative_completeness` و`boundary_quality` بديلين مركزيين، وإعادة تطبيع آمنة عند التجاوز البيئي، وبصمة أوزان.
- **`process_segments` لا يعدّل قائمة المتصل**: `list(raw_segments)` قبل الفرز/التعديل.
- **التطبيع العربي بثلاثة مستويات**: display (كما هو)، comparison، search — مع `dominant_language` للنص المختلط ودعم RTL اختياري، دون المساس بالأسماء/الأرقام/النص المعروض.
- **البصمة**: إضافة بصمة نص التفريغ وبصمة الأوزان وإصدار مخطط العنوان إلى `selection_config` وبصمة الإعدادات — أي تغيير في النص/الأوزان يعيد التوليد.

## الاختبارات

- ملفات جديدة: `tests/test_v741_title_validation.py` (24)، `tests/test_v741_segment_validator.py` (46)، `tests/test_v741_selection_hardening.py` (18) تغطي الحالات الـ18 المطلوبة: تثبيت حد رقمي صريح داخل كلمة، نافذة رقمية كاملة مُتحقَّق منها، رفض رقم/ادعاء/إطار فيديو كامل في العنوان، واقعية العنوان العربي والمختلط، رفض/استعادة النوافذ المعكوسة، عدم تمديد القصير بمحتوى غير مرتبط، عدم اعتبار الدرجات الناقصة موثّقة، كشف التكرار الدلالي البعيد، عدم تعديل قائمة الإدخال، رفض `end_time` المفقود ونص التفريغ الفارغ، توافق JSON القديم، بقاء بوابات الأمان، واستمرار الواجهة/الحرق/التصدير/المراجعة. **CI أخضر: 1405 اختباراً ناجحاً و0 فشل على Python 3.10 و3.11 و3.12**، مع `ruff check .` نظيف و`pip-audit` بلا ثغرات على تبعيات المشروع.

# إصدار OUSSAMA Cutter 7.40.0-pro — جودة اختيار المقاطع: تقييم من 11 عاملاً، حدود كلمات آمنة، عناوين واقعية، وإزالة تكرار دلالية

## المشكلة التي كانت تُفسد المقاطع المختارة

- **الترتيب كان يثق بتقدير النموذج لنفسه**: `selection_score` القديم كان يمنح 40% من وزنه لدرجة `score` التي يكتبها الـ LLM عن نفسه، وأي مكوّن تحريري ناقص كان يُنسخ من نفس الرقم — فلا إشارة حقيقية من النص الفعلي تؤثر في الترتيب.
- **القصّ كان يبدأ/ينتهي وسط الجملة**: المحاذاة كانت تعتمد فواصل صمت 0.35 ثانية فقط، وطوابع الكلمات (word-level) الموجودة في `input.json` لم تكن تُستخدم إطلاقاً، ولا يوجد أي تعامل مع بدايات الربط العربية (لكن، لذلك، ثم…) أو النهايات المعلقة (حرف جر أو عطف).
- **العناوين تهلوس**: العنوان كان يُولَّد من سياق المقطع الكامل ولا يُتحقق منه أبداً ضد النص الفعلي داخل نافذة القص — أرقام وأسماء ونتائج مخترعة كانت تمر للنشر.
- **التكرار**: إزالة التكرار كانت زمنية فقط (تداخل النوافذ)، فنفس الفكرة بصياغة مختلفة تخرج كمقطعين.
- **نتائج قديمة تُعاد**: بصمة الإعدادات لم تكن تشمل وضع viral، ولا محرك الـ AI، ولا النموذج، ولا إصدار البرومبت.

## الحل (متوافق رجعياً بالكامل — لا كسر لأي سلوك قائم)

- **`scripts/clip_scoring.py` (جديد)** — التقييم المركزي: كائن أوزان واحد موثّق (`hook_strength 0.20، standalone_context 0.18، emotional_value 0.15، information_density 0.12، completion_score 0.12، transcript_alignment 0.10، audio_quality 0.05، visual_quality 0.04، title_relevance 0.04` − جزاء التكرار − جزاء الأمان)، قابل للتجاوز عبر `VIRALCUTTER_SELECTION_WEIGHTS`، وعتبة اختيارية `VIRALCUTTER_MIN_FINAL_SCORE` تُرجع **مقاطع أقل وأقوى** بدل حشو ضعيف. كل عامل له بديل حتمي من نص النافذة نفسه عند غياب تقدير الـ LLM.
- **`scripts/transcript_window.py` (جديد)** — تحليل نافذة النص بالضبط: أول/آخر جملة منطوقة، السياق قبل/بعد، الصمت البادئ/المنتهي، نسبة الكلام، اكتمال الفكرة. إصلاح الحدود إنتاجياً: قصّ على حدود الكلمات الفعلية عند توفرها، التقاط الجملة السابقة عند البدء بحرف ربط، إكمال الجملة عند الانتهاء بحرف جر، ولفّة قبلية/بعدية قابلة للضبط (`VIRALCUTTER_PRE_ROLL` 0.20–0.50، `VIRALCUTTER_POST_ROLL` 0.30–0.80) لا تتجاوز الحد الأقصى للمدة أبداً.
- **`scripts/title_factual.py` (جديد)** — تحقق واقعي من العناوين: كل رقم واسم في العنوان يجب أن يظهر في نص المقطع نفسه، وأنماط التهويل (لن تصدق، سر خطير، اكتشاف صادم…) تُجازى دائماً وتُرفض عند ضعف الدليل. العنوان الأساسي يُختار بالدقة الواقعية أولاً، وعند فشل كل المرشحين يُستخدم عنوان وصفي محافظ من كلمات المقطع نفسه **بثقة منخفضة** بدل اختراع عنوان مثير. لغة العنوان تُطابق لغة المقطع (عربي↔عربي).
- **إزالة تكرار دلالية**: تشابه النصوص بعد التطبيع العربي الآمن مع حارسين ضد الإيجابيات الكاذبة (اختلاف الأرقام المذكورة ≠ نفس الفكرة؛ تشابه حرفي بلا تطابق كلمات ≠ تكرار) — النسخة الأقوى فقط تبقى، والمتشابهة جزئياً تُجازى في `repetition_penalty`.
- **JSON موسّع بشكل إضافي**: `transcript_text، hook_text، completion_status، score_breakdown (العوامل الـ11)، quality_flags، rejected_reasons، title_data (العنوان الأساسي + بدائل منمّطة + validation + ثقة)` — الملفات القديمة تُقرأ كما هي.
- **بصمة إعدادات أشمل** في `main_improved.py`: تشمل الآن وضع viral/themes، محرك ونموذج الـ AI، نموذج التفريغ، scene-snap، وإصدار التقييم والبرومبت — أي تغيير يُعيد التوليد تلقائياً، مع `--force-regenerate` كاسم صريح لـ `--force-new-segments`.
- **`prompt.txt`** يُلزم النموذج: عناوين واقعية من نص المقطع فقط، التمييز بين نص المقطع والنص الكامل، JSON فقط، وارفض بدل التخمين.

## الاختبارات

- +74 اختباراً جديداً (`tests/test_v740_clip_quality.py`) تغطي المجالات الـ26 المطلوبة: حدود الكلمات، اكتمال الجمل، المدد، الطوابع غير الصالحة، الصمت الطويل، التكرار الزمني والدلالي، واقعية العناوين ورفض المهلوس، التطبيع العربي وRTL والنص المختلط، حد طول العنوان دون كسر كلمات، الثقة المنخفضة، بصمة الإعدادات و--force-regenerate، التوافق مع JSON القديم، متحدث واحد/متعدد، جمل طويلة، وقفات طويلة، محتوى ثنائي اللغة، عقود مسارات الترجمة والتصدير، وسلامة مراحل الأمان. **CI أخضر: 1317 اختباراً ناجحاً و0 فشل على Python 3.10 و3.11 و3.12**، مع `ruff check .` نظيف و`pip-audit` بلا ثغرات معروفة.

# إصدار OUSSAMA Cutter 7.36.0-pro — ربط الصوت بالوجه من الواجهة: ميزة v7.35 تخرج من سطر الأوامر إلى WebUI

## الميزة كانت حبيسة الـ CLI

- **v7.35 أضاف نواة ربط الصوت بالوجه** (`scripts/voice_face_link.py` + `speaker_diarization.py`): الكاميرا تتعلم أثناء القص أي track من الوجوه يملك المتحدث النشط (فترات كلام من طاقة الصوت + ارتباط حركة الفم؛ pyannote اختياري) وترجّح إطار القص نحوه — لكن التفعيل كان فقط عبر `--voice-face-link on` أو متغير بيئة، **والواجهة التي يعمل عليها المستخدم يومياً لا تستطيع تشغيله**.

## الجديد: مفتاح في إعدادات الوجه المتقدمة

- **الواجهة تكتسب «تعلّم أيّ وجه يتكلّم (ربط الصوت بالوجه)»**: مربع اختيار جديد داخل `Advanced Face Settings ← Experimental: Active Speaker & Motion` في تبويب الإنشاء (والمراجعة/الدفعة)، قيمته الافتراضية `off` تماماً مثل الـ CLI — بدون تفعيل صريح لا يتغير أي سلوك.
- **يمر عبر كل مسارات التشغيل الثلاثة**: زر التشغيل المباشر، إعادة قص مشروع قائم من تبويب المراجعة، والرفع الدفعي (Batch) — `webui/pipeline.py` يضيف `--voice-face-link on` للـ CLI فقط عند التفعيل.
- **توافق الطابور محفوظ**: المعامل الجديد يُلحق في نهاية ترتيب معاملات `run_viral_cutter` بعد `auto_upload_oauth_file`، فخطط Batch المحفوظة قبل التحديث (`.batch_queue.json`) تستأنف بأمان بسلوكها الافتراضي دون إزاحة أي موضع.
- **ترجمة كاملة**: مفتاحان جديدان في اللغات الأربع (عربي/إنجليزي/برتغالي/تركي) بتطابق مفاتيح تام (672 مفتاحاً في كل ملف).
- **بطاقة «ما الجديد»** في الرئيسية عُدّلت لتعكس آخر الإصدارات (ربط الصوت بالوجه في الواجهة، رفع الدفعة، دروس القناة، بوابة الجودة، اعتمادية v7.32).

## الاختبارات

- +5 اختبارات (`tests/test_v736_voice_face_link_ui.py`): `build_command` يضيف `--voice-face-link on` عند التفعيل ولا يضيفه عند الإغلاق/الافتراضي، ويبقى `--focus-active-speaker` مستقلاً عنه، وبطاقة «ما الجديد» تذكر ربط الصوت بالوجه. المجموع 1243 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.35.0-pro — ذكاء المتحدث: الكاميرا تتعلم صوتياً أي وجه يتكلم

## المشكلة التي كان يعانيها تتبع المتحدث

- كاميرا «المتحدث النشط» كانت تعتمد على **حركة الفم فقط** (MAR) + طاقة الصوت **العامة** للمقطع كله — فلا تستطيع معرفة **مين** اللي بيتكلم عندما يتكلم شخصان: أي وجه يتحرك فمه أثناء الصوت يحصل على نفس التعزيز، فتتردد الكاميرا بين الاثنين أو تنقسم بلا سبب.

## الحل: ربط الصوت بالوجه (opt-in، بدون اعتماديات إجبارية)

- **طبقة تحديد «متى يتكلم أحدهم»** (`scripts/speaker_diarization.py`):
  - `detect_speech_turns` — كشف فترات الكلام نقيّة من طاقة الصوت لكل إطار (لا أي مكتبة إضافية): دمج الفجوات القصيرة، تجاهل ضجيج النبضات (أقل من 0.8 ثانية).
  - `diarize_audio` — تفريغ المتحدثين الحقيقي عبر **pyannote** عندما يكون مثبتاً ومعه توكن Hugging Face (`PYANNOTE_AUTH_TOKEN`/`HF_TOKEN`) → يقول «مين اللي بيتكلم» فعلاً؛ يرجع None بهدوء بدونه فينزل المسار لطبقة الطاقة.
- **الربط الصوت-بالوجه** (`scripts/voice_face_link.py` — نقي، مختبر بالكامل): `VoiceFaceLink` يجمع إحصائياً تراكميّاً أثناء العرض نفسه: أي track من الوجوه فمه مفتوح أثناء فترات كل متحدث — فبعد ثوانٍ قليلة يعرف «track 3 هو فم SPEAKER_01» بثقة، و**لا يتدخل أبداً** في القرار عند نقص الدليل أو التعادل (يترك الهيستيريسيس يعمل).
- **التأثير على القص (مقصور):** عند ثقة واضحة، وجه المتحدث الحالي يحصل على +2 نقطة نشاط والمنافس −0.75 (سقف 20) — مجرد «ترجيح» لا «سطو» على القرار.
- **التفعيل:** `--voice-face-link on` في CLI أو `VIRALCUTTER_VOICE_FACE_LINK=1`. الافتراضي `off` — السلوك الحالي لا يتغير إطلاقاً بدون تفعيل صريح.

## الاختبارات

- +16 اختباراً جديداً (`tests/test_v735_speaker_link.py`): كشف فترات الكلام (دمج الفجوات، تجاهل الضجيج، الصمت)، ربط الصوت بالوجه (تعيين المتحدث، التعادل، غياب الدليل)، مساعد الترجيح على وجوه حقيقية بدرجات (سقف، لا تغيير، وجه واحد). المجموع 1238 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.34.0-pro — واجهة مطوّرة: رفع الدفعة، بطاقة «ما الجديد»، ولمسة بصرية متكاملة

## رفع كل المقاطع كدفعة من الواجهة

- **كان الرفع الفردي فقط في الواجهة:** لرفع كل مقاطع المشروع كان عليك الخروج لسكربت. الآن زر **«🚀 رفع كل المقاطع (دفعة كاملة)»** في تبويب النشر مع حقل فاصل زمني اختياري:
  - فاصل = 0 → رفع فوري متتابع لكل مقاطع `final/` (أو `final_polished/`) عبر نفس بوابة الأمان، مع سجل مباشر يتحدث أثناء العمل.
  - فاصل > 0 → جدولة تلقائية على أوقات متباعدة (تتطلب YouTube Private، وتُرفض برسالة واضحة إن لم تكن).
  - المنصات الثانية (TikTok/Reels/IG) تمرر `variant_policy=auto` تلقائياً: أي مقطع سبق نشره على يوتيوب يُرفع كنسخة مختلفة — الحماية من المحتوى المكرر أصبحت سلوك الدفعة الافتراضي وليس خطوة يدوية.

## الرئيسية: بطاقة «ما الجديد» + نظام أساسي أنظف

- **بطاقة ✨ في تبويب Home** تختصر أبرز ما وصل في آخر 5 إصدارات (اختيار يتعلم من نتائج قناتك، دروس القناة بالعربية، نسخ مختلفة لمنصة ثانية، هاشتاغات/صور مصغرة تصل للرفع…) — الزائر الجديد يفهم قيمة الأداة من أول شاشة.
- **لمسة بصرية:** التبويبات أصبحت حبوباً (pills) بحدود دائرية مع توهج برتقالي عند التمرير والتحديد المتدرج، أزرار Secondary بإطار دافئ، سكرولبار موحّد بلون العلامة، وبطاقات متابعة/أخطاء بحدود أخف — كلها CSS فقط، لا تغيير في السلوك.

## الاختبارات

- +3 اختبارات (tests/test_v734_ui_shell.py) + تحقق استيراد كامل للواجهة. المجموع 1222 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.33.4-pro — الواجهة تتكلم: دروس قناتك في الواجهة، زر «ضع أفضل عنوان»، وترجمة التسميات الجديدة

## الواجهة تعرض ما تعلمته الأداة (لا CLI فقط)

- **زر «📊 دروس قناتي (من التحليلات)»** في تبويب النشر/أدوات SEO: يقرأ `performance_insights.json` ويعرض بالعربية خلاصة ما تعلمته الأداة من فيديوهاتك — أفضل صيغة خطاف/زاوية/موضوع (بنسبة تفوقها عن معدلك)، أفضل صيغة عنوان (سؤال/رقم/كلمة خطاف) بمتوسط المشاهدات معها وبدونها، وأفضل ساعات النشر المقاسة. لا يوجد ملف تحليلات بعد؟ النص يشرح خطوتي التفعيل (Analytics API ثم `python -m scripts.performance_loop --project ...`).
- **زر «✅ ضع أفضل عنوان في حقل النشر»**: أدوات SEO كانت تولّد العناوين وتطبعها كنص فقط — الآن بضغطة تولّد 6 عناوين وتضع الأعلى درجة في حقل العنوان مباشرة (تعديل الواجهة فقط، العنوان يبقى قابلاً للتعديل يدوياً).

## شفافية المراجعة

- **علامة «التقييم التحريري مفقود»:** جدول مراجعة المقاطع الآن يضيف بجانب سبب الاختيار، لأي مقطع لم يرسل الذكاء الاصطناعي تقييمه الذاتي (hook/clarity/…) علامة صريحة أنه منسوخ من درجة الانتشار — لا تُقرأ القيم كقياسات حقيقية بعد الآن.

## الترجمة

- **كل التسميات الجديدة من الجولات 7.33.0-7.33.3 مترجمة الآن** في اللغات الأربع (عربي/إنجليزي/برتغالي/تركي): عناصر اختيار A/B، زر أفضل وقت، دروس القناة، ورسائل الحفظ/الأخطاء — 17 مفتاحاً جديداً في كل ملف مع الحفاظ على تطابق المفاتيح (اختبار i18n أخضر).

## الاختبارات

- +4 اختبارات جديدة (`tests/test_v733_4_channel_lessons.py`). المجموع 1219 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.33.3-pro — الاختيار يتعلم من نتائج قناتك + فحص «مختلفة كفاية؟» قبل النشر لمنصة ثانية

## الصيغ الرابحة تُطبَّق على الاختيار تلقائياً (إغلاق حلقة التعلّم)

- **التحليلات كانت تقيس والاختيار لا يسمع:** v7.33.2 أضاف `content_insights` (أي hook_type/angle/topic وصيغ عناوين تجلب مشاهدات على قناتك) لكن `process_segments` لم يكن يستخدمها. الآن `performance_weights` يترجمها إلى **style bonuses محدودة** (`style_bonuses`/`style_bonus_for`): صيغة لا تتفوق على معدلك بـ 15% على الأقل لا تحرك شيئاً، ولا صيغة واحدة تزيد/تنقص أكثر من 3 نقاط من درجة الاختيار — النواة التحريرية تبقى هي المسيطرة.
- **متى يشتغل:** بمجرد وجود `performance_insights.json` بعينات ≥ 3 (أي بعد تشغيل `python -m scripts.performance_loop --project X` مع تفعيل Analytics) — حتى لو لم تصل أي علاقة رقمية لعتبة التقرير بعد، تعلّم المحتوى يطبق.
- **أين يظهر:** `selection_score` الجديد + `style_bonus` و`style_basis` في `selection_breakdown` لكل مقطع — نفس الدرجة التي يعرضها جدول المراجعة ويرتب بها التصدير. المقطع اللي صيغته (سؤال/قصة/موضوع…) أثبتت نجاحها على قناتك يرتفع تلقائياً فوق منافسه المتساوي تحريرياً.

## فحص «مختلفة كفاية؟» قبل رفع النسخة على المنصة الثانية

- **كانت النسخة تُصنع دون تحقق:** `platform_variant.maybe_variant` يطبق التحويلات ويرفع — لكن لم يكن أحد يقيس إن كانت النسخة *فعلاً* مختلفة عن الأصل. الآن بعد كل تحويل تُقارن النسخة بالأصل ببصمة بصرية (`originality.compare_clips`): لو التشابه > 0.50 تُجرَّب بذرة جديدة (حتى 4 محاولات) وتُرفع أول نسخة مختلفة فعلاً.
- **لا حظر صارم أبداً:** لو استُنفدت المحاولات وبقيت النسخ متشابهة، تُرفع أفضل محاولة مع سطر تحذير صريح «visual similarity X بعد 4 محاولات — راجع قبل النشر» (و`VIRALCUTTER_VARIANT_VERIFY=0` يلغي الفحص). غياب OpenCV = قبول النسخة الأولى كأفضل جهد (لا قياس = لا منع).

## الاختبارات

- +11 اختباراً جديداً (`tests/test_v733_3_style_selection.py`, `test_v733_3_variant_verify.py`). المجموع 1215 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.33.2-pro — الأداة تتعلم أي صيغة عناوين/مواضيع تجلب المشاهدات + بوابة جودة للمقاطع الضعيفة

## تحليلات المحتوى: «ليه الفيديو ده نجح؟»

- **حلقة التعلّم كانت عمياء عن المحتوى:** `performance_loop` كان يحسب ارتباط 7 ميزات رقمية فقط (hook/clarity/novelty/… وأفضل ساعات) بالمشاهدات — لا يعرف شيئاً عن *ماذا* كان الفيديو. البيانات الجديدة المسجلة في v7.33.1 (`hashtags/topic/angle/hook_type`) كانت راكدة بلا مستهلك.
- **الآن التقرير يضيف `content_insights`:** لكل `hook_type`/`angle`/`topic` (بعينات ≥ 2): متوسط المشاهدات ونسبة الفرق عن المعدل العام — فتقول مثلاً «hook_type=question: متوسط 950 مشاهدة (+79% عن المعدل)». ولصيغ العناوين (سؤال ؟/رقم/كلمة خطاف مثل كيف/لماذا/how/why): متوسط مشاهدات مع الصيغة وبدونها ونسبة الفرق. وأخيراً ارتباط عدد الهاشتاغات بالمشاهدات (r + قوته).
- **مخرجات إنسانية:** أسطر insights بالعربي/الإنجليزي في تقرير `performance_insights.json` وفي CLI (`python -m scripts.performance_loop --project X`) — نفس مكان «أفضل ساعات النشر» الحالي. خطوة تمهيدية لمحرك اختيار يقترح الصيغ الرابحة تلقائياً في الجولة القادمة.

## بوابة جودة المقاطع (لا تصدير للمقاطع الضعيفة حقاً)

- **كان كل مرشح راسي ينجو:** لا حد أدنى لخطاف/اكتمال السرد/الوضوح؛ المقاطع الضعيفة كانت تُصدَّر مع تحذير وحسب. الآن `apply_quality_gate` (مدمجة في `process_segments` قبل إزالة التكرار) تُسقط أي مرشح حمل **تقييماً ذاتياً حقيقياً** تحت الأرضية (خطاف < 20، سرد < 20، وضوح < 20 افتراضياً) مع سطر WARN باسمه وأسبابه.
- **لا تخمين على غير المُقيَّم:** المقاطع التي لم يرسل النموذج تقييمها (`quality_missing`) لا تُسقط أبداً — قيمها منسوخة من درجة الفيروسية وليس فيها ما يُقاس.
- **ضبط كامل:** `VIRALCUTTER_QUALITY_GATE=0` يلغي البوابة، و`VIRALCUTTER_MIN_HOOK` / `VIRALCUTTER_MIN_NARRATIVE` / `VIRALCUTTER_MIN_CLARITY` تغيّر الأرضيات.

## الاختبارات

- +10 اختبارات جديدة (`tests/test_v733_2_quality_gate.py`, `test_v733_2_performance_content.py`). المجموع 1204 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.33.1-pro — اختيار A/B يُحفظ فعلاً، صورة مصغرة تُرفع مع الفيديو، وزر أفضل وقت للنشر

## اختيار عنوان A/B (كان يُعرض ثم يُرمى)

- **الاختيار صار يُحفظ:** جدول المراجعة كان يعرض بدائل A/B كنص للقراءة فقط وأي تعديل في الخلايا كان يُتجاهل. الآن شريط «مقطع رقم → عنوان A/B → اعتمد» في تبويب Review Segments: يقرأ الخيارات التي عرضها الذكاء الاصطناعي فعلاً لهذا المقطع، يحفظ الاختيار في `recommended_title` + `title_chosen` (الوظيفة النقية `choose_title`/`title_choices_for` في `webui/segments_review.py`)، ويعيد تحميل الجدول مع الحفاظ على علامات ✓/✗ — العنوان المعتمد هو نفسه الذي يفحصه Upload Gate ويصدّره ويُرفع.
- **الحماية من العنوان الغريب:** الوظيفة تقبل فقط عناوين المقطع نفسه أو بدائله — لا يمكن أن ينحرف الاختيار عن محتوى المقطع.
- **إصلاح صامت مرتبط:** أي كتابة لملف `viral_segments.txt` (تطبيق الاختيار أو اعتماد عنوان) كانت تعيد كتابة `{"segments": [...]}` فقط فتُسقط بيانات `source_meta` (بصمات المصدر والإعدادات) من أعلى الملف — الآن كل الكُتّاب يحافظون على الحمولة الكاملة.

## الصورة المصغرة تصل إلى الفيديو على YouTube

- **ثغرة v7.23 أُغلقت:** مولّد الصور المصغرة كان أداة CLI معزولة؛ لا يوجد أي استدعاء `thumbnails.set` في مسار الرفع. الآن بعد نجاح رفع الفيديو يبحث المُرفِّع عن صورة مصغرة موجودة في `project/thumbnails/` (باسم المقطع أو رقمه)، وإذا لم توجد يولّد واحدة تلقائياً من أول إطار (تعطيل كامل عبر `VIRALCUTTER_UPLOAD_THUMBNAIL=0`)، ثم يرفعها عبر `YouTubeUploader.attach_thumbnail` (نفس نطاق OAuth للرفع — لا إعداد إضافي).
- **لا تمنع النشر أبداً:** أي فشل (ملف تالف، ffmpeg/Pillow غائب، رفض API) يظهر كسطر ⚠️ في السجل والرفع مكتمل.

## أفضل وقت نشر بضغطة واحدة

- زر «🕐 Fill best publish time» بجانب حقل الجدولة: يملأ أقرب موعد مناسب قادم حسب المنصة (جداول v7.22 دون إنترنت) مع ملاحظة توضيحية.

## تغذية حلقة التعلّم بالمحتوى

- `publish_history.record` يخزن الآن `hashtags`/`topic`/`angle`/`hook_type`/`variant_of` لكل حدث نشر — كانت حلقة `performance_loop` تقيس أرقاماً فقط (7 ميزات رقمية) ولا تستطيع الربط بين *محتوى* العنوان/الموضوع والمشاهدات؛ السجل الجديد يفتح ذلك دون كسر أي مُستهلِك قديم.

## الاختبارات

- +22 اختباراً جديداً (`tests/test_v733_1_review_titles.py`, `test_v733_1_publish_flow.py`, `test_v733_1_uploader_thumbnail.py`). المجموع 1195 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.33.0-pro — اختيار احترافي للمقاطع، هاشتاغات وSEO تصل إلى الرفع، قاعدة معجم بسياسة ذكية، ونسخ مختلفة عند إعادة النشر لمنصة ثانية

## اختيار المقاطع (احترافية بلا مشاكل)

- **قصّات وسط الجمل بعد ضبط المدة:** النوافذ الصريحة من الذكاء الاصطناعي التي يحرّكها الضبط (تمديد لبلوغ الحد الأدنى، أو اقتطاع لتجاوز الحد الأقصى، أو تثبيت عند حافة التفريغ) كانت تُترك بحواف خام قد تقطع منتصف الكلمة — `process_segments` كان يتجاوز الـ snap لأي نافذة صريحة تماماً حتى لو تحركت حافتها. الآن: أي حافة حرّكها الضبط تُثبَّت على حدود الجمل تلقائياً، بينما تبقى النوافذ الصريحة النظيفة كما هي (العقد الموثق محفوظ).
- **النوافذ المعكوسة (النهاية قبل البداية):** كانت تُنقل بصمت إلى موضع بلا معنى (مقطع 0.1 ثانية). الآن تُقلب الحافتان إلى الترتيب الصحيح (المسافة بين المرسَيْن هي مقصد النموذج الحقيقي) مع سطر تحذير واضح.
- **حلقة التعلّم من الأداء كانت تقرأ من المجلد الخطأ:** `performance_weights.load_weights(os.getcwd())` كانت تبحث عن `performance_insights.json` في مجلد التشغيل بدل مجلد المشروع الذي يكتبه `performance_loop` — فتسقط بصمت إلى الإعدادات الافتراضية في CLI/WebUI. الآن يُمرَّر `project_folder` من كل مستدعي، وأصبح **`title_boost`** (المرتبط بارتباط جودة العنوان بالمشاهدات) يُطبَّق فعلياً على `selection_score` مع تسجيله في `selection_breakdown` (مع `duration_bonus`).
- **شفافية تقييمات الذكاء الاصطناعي:** عندما لا يرسل النموذج تقييمه الذاتي (hook/clarity/completeness/novelty) كانت القيم تُنسخ صامتة من درجة الفيروسية فتبدو كقياسات حقيقية. الآن يُضاف علم `quality_missing` + `missing_components` لكل مقطع لينتبه لها أي واجهة/مرحلة لاحقة.
- **ترقيم العرض بعد الحذف اليدوي:** `apply_selection` في مراجعة المقاطع كان يترك `candidate_rank` متباعداً بعد إلغاء صفوف، فيخالف الترتيب المعروض ترتيب التصدير. أُضيف `_renumber_candidate_ranks` يعيد ترتيب المحتفظ به (المرتّب أولاً ثم legacy حسب selection_score) ويرقّم 1..N — ما تراه في الجدول هو بالضبط ما يُصدَّر.

## النشر: الهاشتاغات والـ SEO تصل فعلاً إلى المنصة

- **هاشتاغات المقاطع كانت تُرمى:** كان `clip_suggestion` يعيد العنوان والكابشن فقط، و`stream_upload_batch` يمرر `[]` دائماً — ففيديوهات YouTube تُرفع بدون `tags` وبدون #hashtags في الوصف رغم أن كل مقطع يحمل 3-5 هاشتاغات من الذكاء الاصطناعي. الآن `clip_metadata` يستخرجها (تطبيع وإزالة # وتكرار، وحد 15 وسماً احتراماً لسقف YouTube)، والدفعة تمررها للمُرفِّع، وسطر «هاشتاغات المقطع» يُعرض في السجل.
- **فحص SEO استشاري قبل الرفع:** عند رفع YouTube يُعرض الآن سطر `[seo]` بتقييم 0-100 للعنوان (محرك v7.22، دون إنترنت) مع تلميحات عملية (الطول الأمثل 35-70 حرفاً، صيغة سؤال/رقم، تجنب التكرار) — تحذير فقط لا بوابة، والسلوك الافتراضي لم يتغير.

## قاعدة معرفة السلامة (قاعدة البيانات القابلة للبحث)

- **المزامنة أصبحت ذكية:** `sync_policy_db` كان يعيد بناء قاعدة `.oussama_policy_lexicon.sqlite3` كاملة (DELETE + إعادة إدراج FTS) في كل `analyze_segments`. الآن بصمة (fingerprint) للمصطلحات الحالية تُحفظ في جدول `lexicon_meta`؛ عندما لا يتغير شيء تُتخطى إعادة البناء (نتيجة `synced: false, reason: unchanged`)، و`--force-sync` يعيد البناء يدوياً. القاعدة أصبحت «knowledge base» تُستعلم ولا تُعاد كتابتها على كل مقطع.
- **فاحص صحة جديد:** `policy_lexicon --doctor` (أو `run_doctor`) يكتشف: تكرارات متعارضة لنفس (المصطلح الطبيعي، اللغة، المصدر) بدرجات/تصنيفات مختلفة (سبب حقيقي للإفراط/التفريط في الحظر)، مصطلحات فارغة، وعدم تطابق فهرس FTS مع الجدول النشط.

## إعادة النشر لمنصة ثانية (تيك توك / ريلز) بلا محتوى مكرر

- **وحدة `scripts/platform_variant.py`:** عند نشر نفس المقطع على TikTok/Reels بعد نشره على YouTube (أو أي منصة أخرى)، تُولَّد نسخة مختلفة فعلياً بمحولات seeded حقيقية من `originality.py` (سرعة ±2%، مرآة، إزاحة قص، معالجة لونية دقيقة) — بدل رفع البايتات نفسها التي تلتقطها خوارزميات «المحتوى غير الأصلي/المكرر». البذرة مشتقة من (الملف + المنصة + عدد محاولات النشر السابقة) فالنسخة قابلة لإعادة الإنتاج وتختلف عند المحاولة الثالثة.
- **السياسات:** `off` (الافتراضي — سلوك سابق محفوظ تماماً) / `auto` (تُنوَّع فقط إذا وُجد نشر سابق ناجح لنفس المقطع على منصة أخرى عبر `publish_history`) / `always`. وضع `dry-run` يعرض القرار فقط دون لمس الملفات، وأي فشل في ffmpeg يتراجع تلقائياً للأصل مع سطر تفسير — لا حظر صارم أبداً.

## تتبع الوجه / المتحدث النشط

- **سجل DEBUG كان يغرق كل إطار:** عند تفعيل `--focus-active-speaker` كانت أسطر `DEBUG:` تُطبع في كل فريم (MAR/الحركة/النتائج + التبديلات) فتملأ سجلات الإنتاج بميغابايتات. الآن كل هذه الأسطر خلف `VIRALCUTTER_DEBUG_FACES=1` عبر `_debug_faces_print`.
- **تعليق مضلل أُصلح:** «Feature currently disabled for stability» كان يصف مسار النشاط بأنه معطّل بينما هو يعمل عند تفعيل الخيار — النص يشرح الآن المسارين بدقة.

## الاختبارات

- +34 اختباراً جديداً (`tests/test_v733_selection.py`, `test_v733_review_renumber.py`, `test_v733_lexicon.py`, `test_v733_platform_variant.py`, `test_v733_publish_flow.py`). المجموع 1173 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.32.4-pro — مزامنة إصدارات README ومنع تقادمها نهائياً

## الإصلاح

- **أسطر «الإصدار الحالي» في الملفات التعريفية الثلاثة كانت متقادمة بستة إصدارات:** `README.md` و`README_en.md` و`README_ar.md` كانت كلها تعرض 7.26.0-pro بينما الكود عند 7.32.x — الزائر الجديد كان يرى رقماً قديماً لا يطابق ما يحصل عليه فعلاً (نفس صنف الانحراف الذي عولج في 7.32.1 لكن في ملفات لم يكن يغطيها الاختبار). الآن الثلاثة محدثة.
- **اختبار يمنع التكرار:** `tests/test_version_consistency.py` كان يربط `app_version.py` و`pyproject.toml` و`changelog.md` فقط؛ الآن يتحقق أيضاً أن سطر «Versão Atual / Current Version / الإصدار الحالي» في كل README يحمل الرقم الحالي — أي تقادم مستقبلي يُسقط CI فوراً.
- **شارات الاختبارات** في الملفات التعريفية الثلاثة: 1138 → 1139.

## الاختبارات

- +1 اختبار جديد (`test_readme_current_version_lines_match_app_version`). المجموع 1139 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.32.3-pro — إنقاذ التفريغ عند استنزاف الذاكرة

## التفريغ الصوتي

- **لا مزيد من الموت الغامض بـ «bad allocation»:** WhisperX + pyannote VAD على فيديو طويل (~72 دقيقة) أو جهاز بذاكرة صغيرة كانا يسقطان بـ `RuntimeError: bad allocation` (ويندوز) أو `Unable to allocate N. MiB` (numpy) — ورسالتان لم تكونا ضمن قوائم كشف OOM، فكان التشغيل يموت كلياً ويُعاد من الصفر ليموت ثانية. الآن:
  - قوائم كشف نفاد الذاكرة في `scripts/oom_guard.py` و`scripts/transcribe_video.py` تشمل `bad allocation` و`bad_alloc` و`unable to allocate` و`_ArrayMemoryError` و`MemoryError` نفسها (رسالته فارغة — كان يفوت دائماً).
- **سلّم إنقاذ داخلي:** عند نفاد الذاكرة أثناء التفريغ: إعادة محاولة بنصف batch (16→8) بعد تحرير ذاكرة CUDA، ثم تحويل تلقائي إلى **backend faster-whisper الخفيف** (CTranslate2 يقرأ الصوت بشكل متدفق ويستعمل Silero VAD الخاص به — بلا pyannote وبذاكرة أقل بكثير) مع كتابة نفس ملفات SRT/TSV/JSON — المحطات اللاحقة لا تتأثر. يُطبع سطر واضح يشرح التحويل وأن خطوة محاذاة wav2vec2 تُتخطى.
- **تشخيص الذاكرة:** يُطبع الآن حجم RAM/VRAM الحر قبل تحميل الصوت/النموذج وعند أي OOM، مع تحذير صريح عندما تكون الذاكرة شبه فارغة (أغلق المتصفح/التطبيقات الثقيلة).

## الاختبارات

- +7 اختبارات جديدة (`tests/test_transcribe_oom_rescue.py`): تغطية OOM markers الجديدة في الحارسين، وإنقاذ faster-whisper (يكتب الملفات والكاش ويعيد المسارات)، وانتشار ImportError عند غياب backend الإنقاذ. المجموع 1138 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.32.2-pro — كشف ffmpeg المكسور قبل التنزيل وإنقاذه تلقائياً

## التنزيل

- **لا مزيد من «ffmpeg is not installed» رغم وجوده:** yt-dlp الحديث لا يكتفي بالبحث في PATH بل *يشغّل* `ffmpeg -bsfs` ويطلب مخرجات إصدار قابلة للتحليل — فنسخة مكسورة على PATH (الحالة الكلاسيكية: `ffmpeg.exe` منسوخ وحده إلى `C:\Windows\system32` بدون ملفات DLL ويحجب تثبيتاً سليماً في `C:\ffmpeg\bin`) كانت تجتاز فحص الوجود ثم تُسقط كل تنزيل مدمج. الآن `scripts/ffmpeg_toolchain.py` يتحقق من التشغيل الفعلي بنفس طريقة yt-dlp.
- **إنقاذ تلقائي:** إذا كانت نسخة PATH مكسورة لكن نسخة سليمة وُجدت بجانب ffprobe العامل (تثبيت ffmpeg الكامل يضع الاثنين معاً) أو في `VIRALCUTTER_FFMPEG_DIR`، يُمرَّر مسارها إلى yt-dlp عبر `ffmpeg_location` ويُطبع سطر توضيحي في السجل — التنزيل يكمل بدل أن يموت.
- **رسالة فشل قابلة للتنفيذ:** عند استحالة الإنقاذ، يتوقف التطبيق قبل التنزيل برسالة تشرح أن النسخة «موجودة لكنها لا تعمل» مع الحلول الثلاثة (حذف النسخة المكسورة / `winget install ffmpeg` / متغير البيئة)، بدل خطأ yt-dlp المضلل. نفس الرسالة تُعرض أيضاً إذا وصل خطأ yt-dlp الأصلي عبر أي مسار آخر.

## الفحص الأولي (Pre-flight)

- **فحص ffmpeg/ffprobe أصبح بالتشغيل لا بالوجود:** `check_ffmpeg_toolchain()` يشغّل الثنائيتين فعلياً — نسخة مكسورة على PATH تظهر الآن ❌ مع سبب دقيق («found at X but it does not run») بدل ✅ كاذبة، ونسخة مُنقَذة تظهر ⚠️ مع موقع البديل.

## الاختبارات

- +8 اختبارات جديدة (`tests/test_ffmpeg_toolchain.py`): الحالة المُبلَّغ عنها (system32 مكسور + إنقاذ من مجلد ffprobe)، الإنقاذ عبر متغير البيئة، الفشل الكامل، ورسائل preflight. المجموع 1131 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.32.1-pro — إصلاح انحراف الإصدارات وتحصين Dependabot

## الإصلاح

- **مزامنة الإصدار عبر المصادر الثلاثة:** كانت `app_version.py` (مصدر الحقيقة المعروض في الـ WebUI) تقول 7.30.0-pro و`pyproject.toml` يقول 7.26.0+pro بينما الكود والسجل عند 7.32 — المستخدم كان يرى رقماً قديماً لا يطابق ما يعمل فعلاً. الآن الثلاثة عند 7.32.1، واختبار جديد `tests/test_version_consistency.py` يمنع تكرار الانحراف مستقبلاً.
- **User-Agent للترجمة لم يعد يتقادم:** `scripts/translate_json.py` كان يرسل ترويسة ثابتة `OUSSAMA-Cutter/7.26`؛ الآن يُشتق الرقم تلقائياً من `app_version.VERSION`.
- **شارات الاختبارات في الملفات التعريفية:** كانت «915 passed» بينما المجموعة الفعلية 1123 — حُدّثت في README الثلاثة.

## Dependabot

- **قواعد `ignore` للتثبيتات الصلبة الموثقة:** أُغلق الباب أمام اقتراحات الترقية التي تكسر المشروع: `numpy>=2` (يكسر whisperx/pyannote)، `onnxruntime>=1.24` (يسقط عجلات Python 3.10)، `huggingface-hub>=1.0` (يكسر مسار تثبيت WhisperX الكلاسيكي). الأسباب موثقة داخل `.github/dependabot.yml` نفسه.

## الاختبارات

- +3 اختبارات جديدة (`tests/test_version_consistency.py`). المجموع 1123 أخضر في هذه البيئة.

# إصدار OUSSAMA Cutter 7.32.0-pro — هوية تنجو بالزمن، ومطابقة عربية أعمق، ومقاطع غير متكررة

## تتبع الوجه

- **الهوية تنجو بالزمن الفعلي لا بعدد دورات الكشف:** كان موت المسار (track) يُحسب بعدد دورات الكشف — فوجه يغيب ~ثانية واحدة في وضع الوجه الواحد (دورات 0.17 ث) كان يفقد هويته ويعود كهوية جديدة، بينما في وضع الوجهين (~ثانية للدورة) كان يعيش 6 ثوانٍ. الآن `max_missing_frames` (5 ثوانٍ × معدل الإطارات) يحكم: متحدث يدير وجهه أو يحجبه شيء للحظات يُستعاد بنفس الهوية مهما تغيّر إيقاع الكشف (وضع 1/2/الفحوص السريعة أثناء grace). الدلالة القديمة بالدورات تبقى الافتراضي لمن يستدعيها.

## اختيار المقاطع

- **تطبيع إملائي عربي للمطابقة الضبابية:** همزات (أ إ آ ٱ → ا، ؤ → و، ئ → ي)، تاء مربوطة (ة → ه)، تشكيل وحركات وتطويل تُشال قبل المقارنة — «الأمور» تطابق «الامور»، والنصوص الإنجليزية تبقى كما هي حرفياً. ألف المقصورة (ى) لا تُدمج مع الياء فتبقى «على» و«علي» كلمتين مختلفتين.
- **مقطعان مُصدَّران لا يتشاركان أكثر من ~60% من اللقطات:** عتبة التكرار الجزئي نزلت من 75% إلى 60% (والإزاحة ≤ ثانية من 50% إلى 45%) — كان ممكن يطلع مقطعان يكرران ثلاثة أرباع المحتوى نفسه. الـ dedup يُبقي الأعلى نقاطاً، والنوافذ المختلفة فعلاً (33% تداخل مثلاً) تبقى كما هي.
- **استرجاع JSON بشكل كائن (`segments: {0:…, 1:…}`):** بعض النماذج ترجع المقاطع ككائن مفاتيحه أرقام بدل قائمة — كان ينتج «صفر مقاطع» بصمت. الآن يُحوَّل الكائن لقائمة مرتبة بمفاتيحه الرقمية.
- **صلة العنوان تُقاس بكلام النافذة الفعلية:** عنوان مُرشَّح كانت تُقيَّم صلته بنصوص الـ LLM نفسها (caption مختلق يرفع الصلة). الآن بعد تثبيت النافذة النهائية تُحسب صلة العنوان بكلمات الترانسكربت داخل [البداية، النهاية] فعلياً (وزن 1.0) مع كلمات الـ LLM غير الموجودة فيها بوزن 0.5 — عنوان يصف محتوى غير موجود في المقطع لم يعد يتفوق.
- **إعادة الاستخدام تتحقق من الإعدادات أيضاً:** بجانب بصمة الفيديو، تُخزَّن بصمة الإعدادات (`config_fp`: عدد المقاطع/الحد الأدنى/الحد الأقصى/chunk/lغة العنوان). تغيير أي إعداد من هذه على نفس الفيديو يفرض إعادة توليد المقاطع بدل إعادة استخدام نوافذ قديمة بُنيت لإعدادات أخرى.

## العناوين

- **`fit_publish_title`:** قصّ العناوين لحدود المنصات على حدود الكلمات لا في منتصفها، مع «…» واحدة عند القص (يوتيوب 100 حرف، تيك توك 150). يُطبَّق عند بناء نص الرفع الفعلي في `upload_gate` و`publish_panel`، ويتعامل مع العربية والمسافات العربية (ZWSP) بشكل صحيح.

## الاختبارات

- +52 اختباراً جديداً (المجموع 1102 أخضر في هذه البيئة): `tests/test_selection_quality_v732.py` (34)، +4 في `test_face_tracker.py`، +14 في `test_title_publish_fixes.py`.

# إصدار OUSSAMA Cutter 7.31.0-pro — هندسة قصّ لا تقصّ الوجه، ومقاطع مثبتة بالمصدر، وعناوين متوافقة مع ما يُنشر

## تتبع الوجه

- **الوجه لا يُقصّ أفقياً بعد الآن (`face_zoom`):** عندما كان `--face-zoom` يضيّق النافذة أكثر من عرض الوجه، كانت الكاميرا «تبتر» الوجه من الجانبين في اللقطات القريبة. الآن يُخفَّض الـ zoom تلقائياً لأقصى قيمة ما زالت تُدخل الوجه كاملاً بهامش 6% — الوجه يظهر أصغر قليلاً لكن سليماً. اللقطات العادية والوضع القديم (`face_zoom=0`) بلا أي تغيير.
- **خلايا الـ Split Screen لا تتبادل الأشخاص:** إعادة الترتيب يسار-يمين داخل `crop_and_resize_multi_faces` كانت تتجاهل ترتيب الخانات الثابت بالهوية، فعندما يتقاطع شخصان أفقياً كانا يتبادلان الخلية العلوية/السفلية في منتصف الحديث. الآن ترتيب الخانات يأتي من الـ tracker وحده (`preserve_order=True`).
- **لا قفزة مفاجئة للـ Split عند ظهور عابر:** قرار 1→2 كان فورياً من كشف واحد، فشخص يمر خلف المتحدث كان يفتح ويغلق تقسيم الشاشة (pop). الآن الالتحام يتطلب دورتي كشف متتاليتين (~0.3 ث) — هستيريسيس متناظرة مع مهلة الهبوط الموجودة.
- **dead-zone يتدرج مع دقة المصدر:** عتبة الاهتزاز الثابتة (40 بكسل) كانت ضعيفة على مصادر 4K. الآن تُضرب في معامل قُطري الإطار (4K ≈ 80 بكسل) وتبقى 1080p وما دونها كما هي تماماً.
- **لا إعادة ترميز بلا داعٍ في reframe:** تحويل مقطع 9:16 إلى 9:16 (عبر CLI) كان يعيد ترميز الملف خسارةً. الآن يُفحص البُعد الفعلي بـ ffprobe ويُتخطى الملف المطابق (`skipped`).

## اختيار المقاطع

- **دعم الأرقام العربية في التوقيتات:** توقيتات LLM عربية مثل `٩٠` أو `٩:٥٠` أو `٩٫٥` كانت تفشل في `int()`/`float()` وتسقط للافتراضي (بداية الفيديو). الآن تُترجم الأرقام الهندية/الفارسية وعلامة `٫` العشرية قبل التحليل.
- **مقاطع بلا أي مرساة لا تُخمَّن:** مرشّح بلا `start_time` ولا `start_text` ولا `end_text` كان يتحول بصمت إلى نافذة (0، الحد الأدنى) — أي مقطع وهمي من أول ثواني الفيديو يزيح مقطعاً حقيقياً. الآن يُسقَط مع تحذير.
- **العبارة المتكررة لا تقصّ المقطع:** عندما يساوي `end_text` سطر البداية نفسه (عبارة يرددها المتحدث)، كان المقطع ينكمش لدقائق ثم يتمدد أعمى. الآن يُبحث عن التكرار التالي بعد سطر البداية وتُستخدم نهايته الحقيقية.
- **ترانسكربت أقصر من الحد الأدنى يُعلَّم بدل الصمت:** المقطع الأقصر من `min_duration` بسبب قصر المادة كان يُصدَّر بصمت. الآن يُعلَّم `under_min` + `transcript_limited` فيظهر في ملف الـ JSON للمراجعة.
- **ترتيب التصدير النهائي يطابق الترتيب المعروض:** اختيار آخر N مقطع آمن كان يفرز بنقطة الـ LLM الخام فيتعارض مع `candidate_rank`/`selection_score` المعروض في جدول المراجعة. الآن يُصدَّر عبر `finalize_top_segments` بنفس الترتيب التحريري.
- **منع إعادة استخدام مقاطع قديمة مع فيديو جديد:** الـ WebUI يعيد الاستخدام تلقائياً (`--skip-prompts`)، فكان تغيير ملف الفيديو داخل نفس المشروع يقطع نوافذ AI قديمة من لقطات جديدة. الآن يُخزَّن `source_meta.source_video_fp` (اسم+حجم+وقت الملف) مع المقاطع، وأي تغيير في المصدر يفرض إعادة التوليد.
- **الترجمة العربية الخارجية لم تعد تُمرَّر على نموذج إنجليزي:** ترجمات SRT/VTT عربية كانت تُجبر على `detected_language='en'` في محاذاة wav2vec2 (نتائج منحرفة). الآن يُكتشف سكريبت النص (`subtitle_language.py`) ويُختار `ar` تلقائياً مع بقاء الـ fallback القديم.
- **قصّ المشاهد المقفول (`--scene-snap`) لا يزيح النافذة:** الـ snap كان يمكن أن يضاعف المدة أو يقصّها خارج الحدود؛ الآن يُقبل فقط ضمن [0.5×، 1.5×] المدة الأصلية وإلا يُرجع للتوقيت المقفول.

## العناوين

- **البوابة تفحص العنوان الذي يُنشر فعلاً:** كانت بوابة الفحص تُقيّم `title` الخام بينما `publish_panel` يرفع `recommended_title` — أي أن النص المفحوص لم يكن النص المرسل. الآن `metadata_axis` و `upload_gate.audit_project` يفحصان `recommended_title` (عبر `effective_title`).
- **كشف Clickbait و engagement bait بالعربية:** قواعد إنجليزية فقط كانت تترك «لن تصدق»، «لا يفوتك»، «شاهد قبل الحذف»، «اشترك في القناة»… تمر. أُضيفت ~14 قاعدة عربية (فصحى + مصرية/دارجة) بنفس آلية `PATTERN_RULES`.
- **فحص تطابق لغة العنوان مع الكابشن:** عنوان إنجليزي مع كابشن عربي (أو العكس) يولّد finding `language_mismatch` (severity منخفضة).
- **الإيموجي المفرط:** أكثر من 3 إيموجي في العنوان+الكابشن يولّد finding `excessive_emoji`، مع أداة `title_text.strip_excess_emoji` للمعالجة لاحقاً.

## الاختبارات

- +100 اختبار جديد: `tests/test_face_crop_geometry.py` (16)، `tests/test_selection_anchoring.py` (47)، `tests/test_title_publish_fixes.py` (37). الإجمالي 1050 أخضر في هذه البيئة (المتبقي يتطلب gradio/PIL في الـ CI).

# إصدار OUSSAMA Cutter 7.30.0-pro — تأطير واعي بالمشاهد وانتقالات سلسة

## تتبع الوجه

- **إعادة ضبط التأطير عند قطع المشهد (scene cut):** عند تبديل الكاميرا كان الـ tracker يبقى متمسكاً بهويات ومواقع من المشهد القديم — الـ EMA smoother يسحب الـ crop من التأطير السابق، والـ fallback المجمّد يمكن أن يحتفظ بشخص غادر الشاشة حتى 3 ثوانٍ. الآن يُكشف كل مقطع مرة واحدة (PySceneDetect أو fallback بالـ OpenCV)، وعند كل قطع تُعاد تهيئة الهويات والـ smoothers والانتقالات ومحدد المتكلم، ويُعاد الكشف فوراً على المشهد الجديد. يمكن إيقافه عبر `--crop-scene-reset off`.
- **انتقالات smoothstep:** حركة الـ crop بين تأطيرين كانت خطية بسرعة ثابتة (إحساس آلي). الآن S-curve (تسارع ثم تباطؤ) تجعل تبديل المتكلمين وإعادة التأطير يبدوان مقصودين واحترافيين.
- **إصلاح `--face-zoom`:** سطر تعريف الـ flag كان غائباً من كوميت 7.29 (الـ getattr بالقيمة الافتراضية أخفى المشكلة) — الآن الـ flag موجود فعلاً في `--help`.

## العناوين

- **كلمات hook بالدارجة المغربية** في مقياس SEO: كيفاش، علاش، شنو، واش، شحال... المحتوى بالدارجة لم يعد يُقيَّم أقل من الفصحى.

## الاختبارات

- 11 اختبار regression جديد في `tests/test_v730_scene_crop.py` (968 المجموع، كلها خضراء).

# إصدار OUSSAMA Cutter 7.29.0-pro — إصلاحات تتبع الوجه واختيار المقاطع والعناوين

## تتبع الوجه

- **هوية واحدة لكل دورة كشف:** كان `FaceTracker.update()` يُستدعى مرتين في كل دورة (قبل وبعد الـ lookahead) بقوائم وجوه مختلفة، فتتشعب الهويات ويقفز الـ crop بين الأشخاص. الآن التحديث المرجعي واحد على القائمة النهائية، وكل المستهلكين (محدد المتكلم، ترتيب الخانات، سجل النشاط) يتشاركون نفس المصدر.
- **إصلاح مطابقة `None == None`:** كان سجل نشاط المتكلم يطابق أول وجه يظهر عند غياب track_id (مثلاً بعد استبدال الـ lookahead)، فيلتصق تاريخ شخص بشخص آخر.
- **سرعة مُطبعة زمنياً في FaceTracker:** السرعة كانت لكل دورة كشف (5 إطارات مقابل ثانية كاملة) فتخطئ التوقعات عند تغيّر الفاصل. الآن بكسل/إطار والتنبؤ يتدرج مع الزمن الفعلي المنقضي.
- **إعادة ترتيب المتكلم بعد الـ lookahead:** عند استبدال قائمة الوجوه بإطار لاحق كان اختيار المتكلم يُرمى silently ويقفز الـ crop لشخص اعتباطي.
- **headroom يعمل فعلاً:** السقف القديم (ربع ارتفاع الوجه) كان يلغي الإزاحة لأي وجه أصغر من نصف الإطار — أي أغلب فيديوهات الـ talking-head. السقف الجديد يحافظ فقط على بقاء الذقن داخل الإطار.
- **`--face-zoom` جديد (اختياري):** تأطير يتكيف مع حجم الوجه فلا يبقى المتحدث البعيد صغيراً في الـ Short (مثلاً `--face-zoom 0.33` للتأطير الكلاسيكي). الافتراضي 0 يحافظ على السلوك القديم.
- الـ lookahead لا يتجاوز عتبة ثقة المستخدم، وأسماء ملفات المخرجات تستخدم `recommended_title` مثل واجهة المراجعة والنشر.

## اختيار المقاطع

- **الترتيب بالنقاط لم يعد يسقط silently:** قيمة score واحدة غير صحيحة (`"95.5"` أو float) كانت تلغي ترتيب القائمة كلها عبر `int()` داخل except صامت.
- **لا تجاوز لنهاية الفيديو:** تمديد الحد الأدنى للمدة أو قصّ الحد الأقصى كان ينتج نوافذ تتجاوز نهاية الترانسكربت (مقاطع فارغة/صامتة في النهاية). الآن النافذة تُزاح للخلف بدل تجاوز الميديا.
- **قصّ على حدود الجمل للحافتين:** الـ snap كان يطبّع بداية المقطع فقط، والنهاية تبقى في وسط الكلمة عند أي تعديل للمدة. الآن النهاية تُكمل الجملة أو تُقصّ عند آخر جملة مكتملة، والنوافذ الرقمية الصريحة من الـ AI تُحترم كما هي.
- **إصلاح ضياع مقاطع كاملة في clean_json_response:** «تطبيع» `\n` قبل التحليل كان يكسر JSON سليماً يحتوي أسطراً جديدة داخل العناوين (مقاطع تضيع بصمت). التحليل الآن على النص الأصلي أولاً، والإصلاح القديم fallback فقط.
- الترانسكربت غير المرتب زمنياً يُرتّب دفاعياً قبل المطابقة.

## العناوين

- **`segment_titles` يرتب فعلاً بالجودة:** مفتاح الترتيب القديم كان no-op (الترتيب الأصلي فريد لكل عنصر) فرغم أن التوثيق يعد «الأقوى أولاً» لم يحدث ذلك قط.
- **توليد عناوين SEO حتمي بين العمليات:** `hash()` المُملّح كان يغيّر «العناوين نفسها لنفس الموضوع» عند كل إعادة تشغيل. الآن seed من SHA-256.
- **كلمات الـ hook بحدود كلمات:** لم يعد «how» داخل «show» يمنح نقاطاً وهمية.

## الاختبارات

- 19 اختبار regression جديد في `tests/test_v729_fixes.py`.
- إصلاح اختبار `test_analyze_report_exposes_best_hours` الهش (كان يفشل على أي جهاز غير UTC).

## صيانة وأمان — 2026-09-05

- أزيلت حزمة `deep-translator` المتضررة أمنياً واستُبدلت بمحول ترجمة صغير يعتمد على `requests` مع مهلة وشكل استجابة متحقق منه.
- أضيف `pip-audit` إلى CI لمنع تمرير تبعية معروفة الثغرات.
- صُححت صيغة اعتماد `g4f` بإزالة extra غير موجود (`all`) وتحديث `uv.lock`.
- أضيفت اختبارات regression للترجمة والـsubtitle segments الفارغة.
